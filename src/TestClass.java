...
public class TestClass {
	...
	public static void main(String[] args) throws TranslateFault {
				...
				try {
					...
					System.out.println("BreakSentences broke up the above sentence into "
		                    + t.breakSentences(breaksentence, "en").length + " sentences.");

		            // Detect Language
					String detect =  t.detect("I have no idea what this language may be");
		            System.out.println("The detected language friendly code is: " + detect);
		            
		            // Detect Language Array
		            String [] detectArray = t.detectArray(textsArray);
					for (int i = 0; i < detectArray.length; i++) {
						System.out.println("The detected languages are: " + detectArray[i]);
					}

		            // App ID Token
					System.out.println("Your tokenized AppId is: " + t.getAppIdToken(5, 4, 300));

		            // Language Names
		    		String[] codeString = { "de", "en", "fr", "ko" };
		            String languageNames[] = t.getLanguagesNames("en", codeString);
					for (int i = 0; i < languageNames.length; i++) {
						System.out.println("Language Name for " + codeString[i] + " is " + languageNames[i]);
					}
		            ...
		            //Translations
		            String [] translations= t.getTranslations("To change this template, choose Tools", "en", "de", 5);
					for (int i = 0; i < translations.length; i++) {
						System.out.println("The matches are :"+translations[i]);
					}
					//Translation Array
					
					String[] translationarray=t.getTranslationsArray(textsArray, "en", "fr", 5);
					for(int i=0;i<translationarray.length;i++)
					{
						System.out.println("The array of matches : "+translationarray[i]);
					}
